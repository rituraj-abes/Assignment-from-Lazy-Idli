...
volatile uint8_t g_ReadDIpinSts;  // Represents real-time status of digital input pins
volatile uint8_t g_AppDIpinSts;   // Represents the application status of digital input pins

// Array to track the consistent count of each digital input pin
static uint8_t pin_consistent_count[8] = {0};
...
int ISR_DIsampling(void) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        uint8_t current_pin_status = (g_ReadDIpinSts >> i) & 1;
        
        // Extract the last known status of the i-th pin from the application status variable
        uint8_t last_known_status = (g_AppDIpinSts >> i) & 1;
